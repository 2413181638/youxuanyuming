...
[ "${CF_SUB_DOMAIN}" = "@" ] || [ -n "${CF_SUB_DOMAIN}" ] || CF_FULL_DOMAIN="${CF_DOMAIN}"
if [ "${SUB_DOMAIN}" = "@" ]; then FULL_DOMAIN="${DOMAIN}"; fi
if [ "${CF_SUB_DOMAIN}" = "@" ]; then CF_FULL_DOMAIN="${CF_DOMAIN}"; fi
...
cf_api() {
  ...
  response="$(curl "${curl_args[@]}")" || {
    log "❌ 调用 Cloudflare API 失败：${method} ${endpoint}"
    return 1
  }
...
get_target_record() {
  ...
  printf '%s' "$response" | jq -c --arg remark "$RECORD_REMARK" '
...
create_record() {
  local ip="$1" payload response record_id
  payload="$(jq -cn \
    --arg domain "$DOMAIN" \
    --arg sub "$SUB_DOMAIN" \
    --arg type "$RECORD_TYPE" \
...
modify_dynamic_dns() {
  local record_id="$1" ip="$2" payload
  payload="$(jq -cn \
    --arg domain "$DOMAIN" \
    --arg sub "$SUB_DOMAIN" \
    --arg line "$RECORD_LINE" \
...
modify_record_remark() {
  ...
  log "✅ 腾讯云已同步记录备注：${RECORD_REMARK} (RecordId=${record_id})"
}

sync_tencent_dns() {
  local current_ip="$1"
  local record_json record_id record_value record_remark

  record_json="$(get_target_record)" || return 1

  if [ -z "$record_json" ] || [ "$record_json" = "null" ]; then
    log "ℹ️ 腾讯云未找到 ${FULL_DOMAIN} [${RECORD_LINE}] 的 A 记录，准备创建"
    create_record "$current_ip" || return 1
    return 0
  fi

  record_id="$(printf '%s' "$record_json" | jq -r '.RecordId // empty')"
  record_value="$(printf '%s' "$record_json" | jq -r '.Value // ""')"
  record_remark="$(printf '%s' "$record_json" | jq -r '.Remark // ""')"

  [ -n "$record_id" ] || {
    log "❌ 腾讯云现有记录缺少 RecordId"
    return 1
  }

  if [ "$record_value" != "$current_ip" ]; then
    log "ℹ️ 腾讯云检测到 IP 变化：${record_value:-<空>} -> ${current_ip}"
    modify_dynamic_dns "$record_id" "$current_ip" || return 1
  else
    log "ℹ️ 腾讯云 IP 未变化：${current_ip}，无需更新"
  fi

  if [ "$record_remark" != "$RECORD_REMARK" ]; then
    modify_record_remark "$record_id" || return 1
  fi
}
...
cf_get_zone_id() {
  ...
  [ -n "$zone_id" ] || {
    log "❌ Cloudflare 未找到 Zone：${CF_DOMAIN}"
    return 1
  }
...
cf_get_record() {
  local zone_id="$1" response
  response="$(cf_api "GET" "/zones/${zone_id}/dns_records?type=${CF_RECORD_TYPE}&name=${CF_FULL_DOMAIN}&per_page=100")" || return 1
  printf '%s' "$response" | jq -c '.result[0] // empty'
}

cf_create_record() {
  local zone_id="$1" ip="$2" payload response record_id
  payload="$(jq -cn \
    --arg type "$CF_RECORD_TYPE" \
    --arg name "$CF_FULL_DOMAIN" \
    --arg content "$ip" \
  ...
  response="$(cf_api "POST" "/zones/${zone_id}/dns_records" "$payload")" || return 1
  record_id="$(printf '%s' "$response" | jq -r '.result.id // empty')"
  log "✅ Cloudflare 已创建记录：${CF_FULL_DOMAIN} -> ${ip} (RecordId=${record_id})"
}

cf_update_record() {
  local zone_id="$1" record_id="$2" ip="$3" payload
  payload="$(jq -cn \
    --arg type "$CF_RECORD_TYPE" \
    --arg name "$CF_FULL_DOMAIN" \
    --arg content "$ip" \
  ...
  cf_api "PUT" "/zones/${zone_id}/dns_records/${record_id}" "$payload" >/dev/null || return 1
  log "✅ Cloudflare 更新成功：${CF_FULL_DOMAIN} -> ${ip} (RecordId=${record_id})"
}

sync_cloudflare_dns() {
  local current_ip="$1"
  local zone_id record_json record_id record_value

  zone_id="$(cf_get_zone_id)" || return 1
  record_json="$(cf_get_record "$zone_id")" || return 1

  if [ -z "$record_json" ] || [ "$record_json" = "null" ]; then
    log "ℹ️ Cloudflare 未找到 ${CF_FULL_DOMAIN} 的 A 记录，准备创建"
    cf_create_record "$zone_id" "$current_ip" || return 1
    return 0
  fi

  record_id="$(printf '%s' "$record_json" | jq -r '.id // empty')"
  record_value="$(printf '%s' "$record_json" | jq -r '.content // ""')"

  [ -n "$record_id" ] || {
    log "❌ Cloudflare 现有记录缺少 id"
    return 1
  }

  if [ "$record_value" != "$current_ip" ]; then
    log "ℹ️ Cloudflare 检测到 IP 变化：${record_value:-<空>} -> ${current_ip}"
    cf_update_record "$zone_id" "$record_id" "$current_ip" || return 1
  else
    log "ℹ️ Cloudflare IP 未变化：${current_ip}，无需更新"
  fi
}

validate_config() {
  local missing=() v
  for v in SECRET_ID SECRET_KEY DOMAIN SUB_DOMAIN RECORD_TYPE RECORD_LINE CF_API_TOKEN CF_DOMAIN CF_SUB_DOMAIN CF_RECORD_TYPE; do
    if [ -z "${!v:-}" ]; then
      missing+=("$v")
    fi
  done
...
show_status() {
  ...
  echo "脚本: ${SCRIPT_PATH}"
  echo "腾讯云域名: ${FULL_DOMAIN}"
  echo "Cloudflare 域名: ${CF_FULL_DOMAIN}"
  echo "线路: ${RECORD_LINE}"
...
run_daemon() {
  ...
  log "=========================================="
  log "腾讯云 + Cloudflare DDNS 后台服务已启动"
  log "腾讯云域名: ${FULL_DOMAIN}"
  log "Cloudflare 域名: ${CF_FULL_DOMAIN}"
  log "线路: ${RECORD_LINE}"
