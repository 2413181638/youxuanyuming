...
RATE_LIMIT_BATCH = 10        # 速率限制：每批处理 10 个请求
RATE_LIMIT_DELAY = 1         # 每批之间暂停 1 秒
MAX_IPS_PER_SUBDOMAIN = 30   # ✅ 每个子域名最多 30 个 IP
...
def get_ip_list(url, max_ips=MAX_IPS_PER_SUBDOMAIN):
    """
    根据 URL 获取 IP 列表，支持 .txt 文件和 HTML 页面解析。
    只保留前 max_ips 个 IP（默认最多 30 个）。
    """
    try:
        ...
        if url.endswith('.txt'):
            ...
            text = soup.get_text()
            raw_ips = re.findall(r'\b(?:\d{1,3}\.){3}\d{1,3}\b', text)
            ip_list = [ip for ip in raw_ips if is_valid_ip(ip)]
        # 截取最多 max_ips 个
...
def update_cloudflare_dns(ip_list, api_token, zone_id, subdomain, domain, ttl=300):
    ...
    def add_dns_record(ip):
        ...
        try:
            _ = request_with_retry('POST', f'https://api.cloudflare.com/client/v4/zones/{zone_id}/dns_records', headers=headers, json=data)
...
async def main():
    ...
    - 每个子域名最多取 30 个 IP（MAX_IPS_PER_SUBDOMAIN）
    - TTL = 300 秒（5 分钟）
    """
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
        for subdomain, url in subdomain_ip_mapping.items():
            if is_timeout():
                break
            # ✅ 每个子域名最多 30 个 IP
            ip_list = await asyncio.get_event_loop().run_in_executor(executor, get_ip_list, url, MAX_IPS_PER_SUBDOMAIN)
            if ip_list:
